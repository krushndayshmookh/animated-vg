<template>
  ...
  <q-btn dense icon="add" label="Add Opacity Anim" data-test="timeline-add-opacity" @click="onAddOpacity" />
  ...
  <q-btn dense icon="stop" label="Stop" data-test="timeline-stop" @click="onStop" />
    ...
    <div class="q-mt-md" data-test="timeline-anim-list">
      ...
      <div v-for="row in animations" :key="row.targetId + ':' + row.animIndex" class="row items-center q-gutter-sm q-mb-xs" data-test="timeline-anim-row">
        <div class="text-caption">#{{ row.targetId }} • {{ row.kind }} • {{ row.attributeName }}</div>
  <input v-model="edits[idKey(row)].from" class="q-input q-field__native" :data-test="'timeline-anim-from-' + row.animIndex" type="text" placeholder="from" />
  <input v-model="edits[idKey(row)].to" class="q-input q-field__native" :data-test="'timeline-anim-to-' + row.animIndex" type="text" placeholder="to" />
  <input v-model="edits[idKey(row)].dur" class="q-input q-field__native" :data-test="'timeline-anim-dur-' + row.animIndex" type="text" placeholder="dur" />
        <q-btn dense label="Save" :data-test="'timeline-anim-save-' + row.animIndex" @click="onSave(row)" />
      </div>
    </div>
  </div>
</template>
...
function onAddOpacity() {
  ...
  store.addOpacityAnimation(store.selectedId, { from: 1, to: 0.2, dur: '1s', begin: '0s', fill: 'freeze' })
}
function onPlay() {
  const el = resolveSvg()
